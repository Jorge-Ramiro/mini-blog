...
@admin_required
def index():
    count_all_posts = Post.get_count_all()
    count_all_users = User.get_count_all()
    return render_template("admin/index.html", count_all_posts=count_all_posts, count_all_users=count_all_users)
...
def post_form():
    ...
    if form.validate_on_submit():
        ...
        # Si el usuario indicó un fichero, el aributo data correspondiente será distinto de None.
        if file:
            image_name = secure_filename(file.filename)
            images_dir = current_app.config["POSTS_IMAGES_DIR"]
            os.makedirs(images_dir, exist_ok=True) # creamos el directorio media/posts si no existe.
            file_path = os.path.join(images_dir, image_name)
            file.save(file_path)
            #file = request.files["post_image"]
            # si el usuario no selecciona un fichero, el navegador enviará una parte vacía sin nombre de fichero
            #if file.filename:
